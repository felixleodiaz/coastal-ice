...
# load in optuna (just for rerunnning)

study = joblib.load("optuna_gee_rf_study.pkl")
print("Optuna study loaded from 'optuna_gee_rf_study.pkl'")
...
eval_classifier = ee.Classifier.smileRandomForest(**final_gee_params)\
    .train(
        features=train_data,
        classProperty=target,
        inputProperties=features
...
# classify the test set (standard integer classes)

class_pred = test_data.classify(eval_classifier)

# create a probability version of the classifier

prob_classifier = eval_classifier.setOutputMode('MULTIPROBABILITY')

# chain the classification

validated_test = class_pred.classify(prob_classifier, 'probabilities')

# TASK 1
# export predictions and probabilities to csv

export_columns = features + [target, 'classification', 'probabilities', 'fold_rand']

task_predictions = ee.batch.Export.table.toDrive(
    collection=validated_test.select(export_columns),
    folder='EarthEngineResults',
    description='TestDataPredictionsRF',
    fileFormat='CSV'
)
...
classifier_asset_id = 'projects/gee-personal-483416/assets/random_forest_seaice_classifier_prob'
